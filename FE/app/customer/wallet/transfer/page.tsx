...
export default function TransferPage() {
    ...
    const TransferMoney = async (amount: number, des: string, walletNumber: string, otp: string[]) => {
        ...
        if (isOtpComplete) {
            ...
            try {
                ...
                if (response.ok){
                    const data = await response.json();
                    router.push("/customer/wallet/transfer/success");
                    console.log(data.data)
                }  else router.push("/customer/wallet/transfer/cancel");
            } catch (error) {
                console.log(error);
            }
        } else {
            setNotificationContent("Vui lòng nhập đủ 6 chữ số OTP!")
    ...
    return (
        <div className={"w-full h-full border rounded-[25px] pb-[20px] flex flex-col items-center justify-center font-sf"}>
            {
                openNotificationConfirm &&
                <ConfirmDialog
                    message={"Xác nhận bạn muốn chuyển tiền"}
                    onConfirm={()=> {
                        setOpenNotificationConfirm(false);
                        TransferMoney(amount, note, walletNumber, otp)
                    }}
                    onCancel={()=>setOpenNotificationConfirm(false)}/>
            }
            {
                openNotification && <AlertMessage message={notificationContent} onClose={()=>setOpenNotification(false)}/>
            }
            ...
            <div className={`w-[400px] h-full mt-[20px]`}>
                ...
                <div className="h-[80px] bg-amber-50 w-full rounded-[20px] border border-amber-200 mb-[20px] px-[20px] flex flex-col justify-center">
                    <p className=" flex items-center text-[15px] text-stone-700">Số Dư Của Bạn</p>
                    {wallet?.balance &&
                        <VndText
                            amount={wallet.balance}
                            classNameCurrency={"font-[400] text-[18px] font-sf text-amber-600" }
                ...
                <div className="h-[40px] w-full flex justify-center items-center mt-[20px]">
                    <button onClick={()=> setOpenNotificationConfirm(true)} className="h-full px-[30px] font-sf text-[15px] text-white bg-amber-600 rounded-full">
                        Chuyển Tiền
                    </button>
                </div>
            </div>
