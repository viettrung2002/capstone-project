            ...
            <div
                ...
                <Image src={product.image} alt={"product_image"} width={1000} height={1000} className={"object-contain"}/>
                    ...
                    <button className={`absolute w-[130px] h-[45px] hover:bg-gray-700 bg-blue-500 rounded-[4px] bottom-[30px] flex items-center justify-center transition-all duration-200 translate-y-[75px] opacity-0 ease-in-out group-hover:opacity-100 group-hover:translate-y-0 `}>
                        ...
                        <p className={`ml-[5px] text-cl-button-text font-fre  `}>Add to cart</p>
                    </button>

                    {product.discount > 0 ? (
                    <div className={`absolute left-0 top-0 w-[50px] h-[30px] bg-red-500 text-gray-50 rounded-[4px] flex items-center justify-center`}>
                        <p className={`font-fre font-[500] text-[15px]`}>-{product.discount}%</p>
                    </div>) : null}


            </div>
            <div className={`pl-[20px] pt-[10px] flex flex-col justify-between`}>
                <p className={`font-fre text-gray-500 `}>{product.category}</p>
                <p className={`font-fre text-cl-text text-[20px] font-[500] `}>{product.name}</p>
                <div className={`flex items-center h-[30px] `}>
                    ...
                    <p className={`font-fre text-gray-500 ml-[10px] `}>{product.star} Review</p>
                </div>

                {product.discount > 0 ? (
                    <div className={`flex items-center`}>
                        <p className={`font-fre text-cl-hover-text text-[20px] font-[600] `}>${product.price - product.price * product.discount / 100}  </p>
                        <div className={`relative ml-[10px]`}>
                            <p className={`font-fre text-gray-400 text-[18px] font-[400] ml-[0px]`}>${product.price}  </p>
                            <div className={`absolute w-full border-t border-gray-400 top-1/2 border-[1/2px]`}></div>
                        </div>
                    </div>
                ): (
                    <div className={`flex items-end`}>
                        <p className={`font-fre text-cl-hover-text text-[20px] font-[600] `}>${product.price }  </p>
                    </div>
                )}
    ...
    return(
        <div className={`col-span-1 aspect-[70/100] rounded-[4px] bg-white shadow p-[6px] relative `}>
            ...
            <div className={` pt-[10px] flex flex-col justify-between items-center `}>
                <p className={`font-fre text-cl-text text-[15px] font-[500] `}>{product.name}</p>
                {product.discount > 0 ? (
                    <div className={`flex-col flex items-center`}>
                        <p className={`font-fre text-cl-hover-text text-[20px] font-[600] `}>${product.price - product.price * product.discount / 100}  </p>
                        <div className={`relative`}>
                            <p className={`font-fre text-gray-400 text-[15px] font-[400] ml-[0px]`}>${product.price}  </p>
                            <div className={`absolute w-full border-t border-gray-400 top-1/2 border-[1/2px]`}></div>
                        </div>
                    </div>
                ): (
